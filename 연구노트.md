...
순수스프링은 확실히 easycompany 플젝 주로 보면 되겠다고 설명, 여기 쳌리는 그래도 부트를 조금 감미한 나만의 쳌리임을 설명하고 시작.
...
이클립스:live플러그인(애초에 톰캣구동하는데 Python이 아닌이상 이건 필요없을듯, 우선 easycompany플젝은 소스 저장시 새로고침하면 자동 반영 되더라..), 실행빨리하는 단축키?, 자동완성(설정으로해결: preference-content assist검색 -> Auto triggers java에 <=$:{.@qwertyuioplkjhgfdsazxcvbnm_QWERTYUIOPLKJHGFDSAZXCVBNM
 , Disable insertion triggers except 'Enter' 체크), 글자크기는(preference-font-basic 12로 함), 힙메모리보기(preference-general-show heap 체크), 한글깨지면 UTF-8인코딩(preference-encoding검색후 Content Types에 Java Class File과 Test에 UTF-8로 Update), tdd등록(Window > Preferences > Java > Editor > Templates > new > Name:tdd, Description:TDD 테스트 메소드 생성, Pattern:원하는 tdd 코드 작성), 자동정렬Ctrl+Shift+F로 제공하는데 html과jsp에서 좀 엉망인걸 해결(Preferences>Web>Html>Editor에서 입맛에맞춰 설정)  
...
=> JPA에서 페이징은 메소드로 제공, 여기선 SQL문으로. 비교해서 알아둬야지.
...
3. **controller + JSP**  
   ...
   @RequestParam, @PathVariable, @ModelAttributes 헷갈릴 때: [쳌리-컨트롤러](https://bh946.github.io/checklist/CHECK_LIST_SPRING/#%EC%8A%A4%ED%94%84%EB%A7%81-%EB%B6%80%ED%8A%B8%EC%9D%98-%EC%95%A0%EB%85%B8%ED%85%8C%EC%9D%B4%EC%85%98%EB%93%A4)  
   결론: @ModelAttributes는 자세한 기능을 알고 사용하자. 웬만하면 GET엔 @RequestParam이나 @PathVariable 쓰자.  
   ...
   예로: fragment -> jsp:include, th:src -> c:url, th:text -> spring:message, text -> c:out, th:each -> c:forEach,  th:if -> c:if, th:error -> 직접 등 코드는 아래 참고
   ...
   **컨트롤러 로직에서 생각할 점:** **-> 개발 후 이거 맞는것 같으면 쳌리에 추가 및 ">"로 위에 추가**
...
2. **기능개발+TEST -> "검색(동적쿼리)+페이징, 자동완성(검색)"**
   ...
   - 서비스 메소드 findAllWithPage, findTotalCount 를 사용해 각각 model, paginationInfo에 담음
     ...
     - <details><summary><b>페이징 컨트롤러 예시 코드</b></summary>
       ...
       public String gallery(@ModelAttribute Item item, Model model) throws Exception {
       //		item.setPageUnit(propertiesService.getInt("pageUnit")); //외부설정
       //		item.setPageSize(propertiesService.getInt("pageSize"));
       	item.setPageUnit(10);
       	item.setPageUnit(10);
       //
       	// pagination setting
       ...
       //
       	...
       	List<Item> items = itemService.findAllWithPage(item);
       	int totCnt = itemService.findTotalCount(item);
       	model.addAttribute("resultList", items);
       	paginationInfo.setTotalRecordCount(totCnt);
       //
       	model.addAttribute("paginationInfo", paginationInfo);
       	// Pagination
...
3. **JSP Ver1: 페이징과 검색란**
   ...
   <details><summary><b>jsp 예시 코드 - 페이징</b></summary>
   <div markdown="1"><br>
   ```jsp
   ...
   검색의 자동완성은 검색란에 검색 시 ajax+jquery의 auto 사용하여 로드
...
6. **JSP Ver2: 현대식으로 AJAX를 추가하여 SPA와 유사한 방식 적용법**

   예전방식은 페이지 자체가 새로고침되는 거라면, 이 방식은 화면의 원하는 부분만 바뀐다.

   기존 코드의 js 부분이랑 바꿀 부분태그에 id만 붙여주면 됨!

   <details><summary><b>AJAX 적용 코드</b></summary>
   <div markdown="1"><br>
...
1. 메시지 국제화 -> XML 연습겸 XML 쓰겠.

   > 순수스프링은 HandlerMapping지정해서 인터셉터 등록하면 해당 핸들러만 사용하다보니 문제없이 정상 동작하지만,  
   > 부트는 항상 여러 HandlerMapping을 자동등록 하기 때문에 순수스프링처럼 지정해버리면 다른 HandlerMapping들에 인터셉터가 등록이 되지 않는 문제가 발생한다.  
   >
   > `<mvc:interceptors>` 패턴을 사용해서 부트(=Java Config)처럼 "모든 핸들러매핑에 인터셉터 등록"하자.

   context-servlet.xml에 messageSource빈과 국제화빈 및 인터셉터를 등록  
...
2. **외부설정(property)** -> properties & java (참고: eGov 라이브러리 지원-EgovPropertyServiceImpl)
   ...
   <details><summary><b>@ConfigurationProperties 사용코드(+외부설정 포함):</b></summary>
   <div markdown="1"><br>
   ...
   **MyDataSourceConfig.java -> @EnableConfigurationProperties로 @ConfigurationProperties를 빈 사용 활성화**<br>참고) 바로 사용해도 되지만 객체로 쉽게 사용할 수 있게 추가로 `MyDataSource.java` 를 빈 등록 했다.
   ...
   <details><summary><b>eGov 사용코드(Java):</b></summary>
   <div markdown="1"><br>
   ```java
   @Configuration
...
6. **DataSource, 로그레벨랑 sql쿼리로그랑 프로필&외부설정 등 자잘한거 properties.xml & java로**

   **xml은? MyBatis, 메시지&국제화, Exception, AOP, Validation-자카트라 커믄 정도려나?**  

<br>

리팩토링 레츠고... 캐시(웹,서버-서비스단에)...

new erdiagram 



1. - **컨트롤러 첨부터 쭉 만들면서 JSP도 첨부터 쭉 만들기~! (이제 어려울거 없음)**
   - **여기까지 하면 MVC 부분은 끝~! 나머지 설정들이랑 리팩토링하면 됨.**

다하면 플젝 lepl, secret도 둘다 다시 복습해두게. 노션이랑 쳌리에 잘 정리된건지도 봐두려고. 확실히 JPA사용했을때랑 MyBatis 사용했을때랑 개발이 좀 다름.(JPA가 훨씬 객체지향적)  
=> 특히, 이 플젝도 따로 노션에 복습해두는게 애초에 나중에 코드 복습하기 좋을듯.



마지막엔 코드 추가 안한곳엔 다 추가해두장!





흐아.. 쳌리 이거 작성하는거 spring.md 오래걸리겠노;  
크게 인텔리J+Gradle+SpringBoot+JPA 로 개발 VS 이클립스+pom.xml+Spring+MyBatis 로 개발 이겠네.  
물론, 오른쪽은 eGov 플젝 얘기들일테고..  
=> 개념은 줄여라. 쳌리는 코드보고 할거고, 개념은 노션 봐야지!





다하면 순수스프링과 부트 차이를 좀 알게끔 "노션"에도 적당히 추가해두자. (아래꺼 꼭 쳌리나 노션에 있어야할 개념인 듯..)

아래 꼭 초기에 언급해두는게 좋을듯?

**순수스프링은 크게 2가지 계층 XML + 젤 최상위 web.xml:**   
이거 그림 쳌리에 같이있었는데 이거랑 + 예시 코드(mvc플젝꺼)를 같이 정리하면 딱일듯.

- 1번: WEB-INF 하위 XML(오른쪽-Child) → 컨트롤러 및 웹 관련 빈 관리
  - 예로 컴포넌트스캔(Controller), mvc:interceptors, mvc:view-controller 등
  - 특히, mvc:view-controller 는 컨트롤러 메소드 없이 **직접 URL을 뷰에 매핑**
- 2번: resources 하위 XML(왼쪽-Root) → 서비스, 리포지토리 및 공통 빈 관리
  - 예로 컴포넌트스캔(Repository, Service) 등

- 최상위: 톰캣이 항상 체크하는 web.xml -> 젤 최상위 설정
  - 예로 필터, 서블릿(ex:디스패처서블릿), 1번과 2번 XML 등록 등

<details><summary><b>예시 코드 XML 3개</b></summary>
<div markdown="1"><br>
...
**순수스프링과 부트에서의 HanderMapping 차이와 "메시지 국제화": 인터셉터 이해**

- **순수스프링**은 HandlerMapping 따로 설정 안하면 자동으로 기본값을 사용한다.  
  단, 메시지 국제화를 위해 HandlerMapping을 지정하여 사용하면 해당 HandlerMapping만을 사용한다.
  - 로그를 찍어보면 실제로 지정한 HandlerMapping 1개만 있었다.
- **부트**는 항상 여러 HandlerMapping을 자동으로 등록한다. (부트는 스프링MVC패턴을 위한 디스패처서블릿 등등 모두 자동으로 등록)
  - 로그를 찍어보면 5개? 정도 찍혔던거 같다.
- **메시지 국제화**는 "핸들러매핑에 인터셉터"를 등록해야 한다.   
  **"디스패처서블릿->HandlerMapping->인터셉터 거쳐서 컨트롤러 진입"** 을 해야하기 때문이다.
  - 따라서 순수 스프링에선 HandlerMapping지정해서 인터셉터 등록하면 해당 핸들러만 사용하다보니 문제없이 정상 동작하지만,
  - 부트를 사용한다면, 순수 스프링처럼 지정해버리면 다른 HandlerMapping들에 인터셉터가 등록이 되지 않는 문제가 발생한다. (물론, 1개 1개 부트가 사용한 핸들러매핑 로그보고 직접 등록하면 되겠지만 비추)
    - 이때, 부트가 권장하는 Java Config 방식을 사용하거나 `<mvc:intecptors>` 패턴을 사용하자.
    - **둘 다 "모든 핸들러매핑"에 인터셉터를 등록해준다!**
